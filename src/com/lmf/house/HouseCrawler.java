...
public class HouseCrawler extends WebCrawler {
	...
	public boolean shouldVisit(Page referringPage, WebURL url) {
		...
		boolean isNeed = true;
//		isNeed = isNeed && !FILTERS.matcher(href).matches();
		isNeed = isNeed && (href.endsWith("html") || isPageHtml(href));
		isNeed = isNeed
				&& (href.startsWith(HouseConstant.SEED_URL_WEB) || href.startsWith(HouseConstant.SEED_URL_MOBILE));
		return isNeed;
		
	}

	private boolean isPageHtml(String str) {
		String[] strArray = str.split("/");
		if (strArray != null && strArray.length > 0) {
			String pg = strArray[strArray.length - 1];
			if (pg.startsWith("pg")) {
				return true;
			}
		}
